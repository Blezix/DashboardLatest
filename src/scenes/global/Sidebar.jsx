...
import { Box, List, ListItem, ListItemText, ListItemIcon, Typography, Button, useTheme, useMediaQuery, Slide } from '@mui/material';
    ...
    return (
        <Slide direction="right" in={true} timeout={500} mountOnEnter unmountOnExit>
            <Box sx={{ width: { xs: '0', sm: '18%' }, height: '100vh', position: 'sticky', top: 0 }}>
                <Box className="TopSidebar" alignItems="center" justifyContent="center" height="15%" color="text.secondary" display="flex" sx={{ flexDirection: { xs: 'row', sm: 'column', md: 'row', lg: 'column' }, padding: { xs: 0, sm: 0 } }}>
                    <Typography variant="h5" sx={{ color: 'white', padding: { xs: 2, sm: 2 } }}>
                        Company
                    </Typography>
                </Box>
